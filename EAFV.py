...
series = pd.read_excel('EAFVdata.xls', header=0, index_col=0).squeeze()
...
series.plot(title="Retail Sales", xlabel="Year", ylabel="EAFV")
pyplot.show()
...
seasonal_data.plot(title="Retail Sales", xlabel="Month", ylabel="EAFV", legend=False)
...
fit2.forecast(11).rename('Model 2: Opt HW-multiplicative seasonality').plot(color='#7BA23F', legend=True)

pyplot.xlabel('Dates')
pyplot.ylabel('Values')
pyplot.title('HW method-based forecasts for EAFV')
pyplot.show()
