...
series = pd.read_excel('JQ2Jdata.xls', header=0, index_col=0).squeeze()
...
series.plot(title="The Manufacturing and Business Sector", xlabel="Year", ylabel="JQ2J")
pyplot.show()
...
seasonal_data.plot(title="The Manufacturing and Business Sector", xlabel="Month", ylabel="JQ2J", legend=False)
...
# In this case, Holt-Winter with additive seasonality have the best behaviour.
fit1.fittedvalues.plot(color='#66BAB7')
series.rename('Time plot of original series').plot(color='black', legend=True)
fit1.forecast(12).rename('Model 1: Opt HW-additive seasonality').plot(color='#66BAB7', legend=True)

pyplot.xlabel('Dates')
pyplot.ylabel('Values')
pyplot.title('HW method-based forecasts for JQ2J')
pyplot.show()
